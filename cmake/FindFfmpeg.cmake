...
if(NOT FFMPEG_LIBRARY_AVFORMAT)
  find_library(
    ...
    NAMES ${GR_THIRDPARTY_LIBRARY_PREFIX}avformat${GR_THIRDPARTY_LIBRARY_SUFFIX} avformat
  )
endif()
...
if(NOT FFMPEG_LIBRARY_AVCODEC)
  find_library(
    ...
    NAMES ${GR_THIRDPARTY_LIBRARY_PREFIX}avcodec${GR_THIRDPARTY_LIBRARY_SUFFIX} avcodec
  )
endif()
...
if(NOT FFMPEG_LIBRARY_SWSCALE)
  find_library(
    ...
    NAMES ${GR_THIRDPARTY_LIBRARY_PREFIX}swscale${GR_THIRDPARTY_LIBRARY_SUFFIX} swscale
  )
endif()
...
if(NOT FFMPEG_LIBRARY_OPENH264)
  find_library(
    ...
    NAMES ${GR_THIRDPARTY_LIBRARY_PREFIX}openh264${GR_THIRDPARTY_LIBRARY_SUFFIX} openh264
  )
endif()
...
if(FFMPEG_VERSION_DIR)
  if(NOT FFMPEG_VERSION_STRING)
    if(EXISTS ${FFMPEG_VERSION_DIR}/libavcodec/version_major.h)
      ...
      string(
        REGEX REPLACE ".*#define LIBAVCODEC_VERSION_MAJOR[ \t]*([0-9]+).*"
        "\\1."
        FFMPEG_MAJOR_STRING
    ...
    string(
      REGEX REPLACE ".*#define LIBAVCODEC_VERSION_MINOR[ \t]*([0-9]+).*"
      "\\1."
      FFMPEG_MINOR_STRING
      ${FFMPEG_VERSION_H_TEXT}
    )
    string(
      REGEX REPLACE ".*#define LIBAVCODEC_VERSION_MICRO[ \t]*([0-9]+).*"
      "\\1"
      FFMPEG_MICRO_STRING
      ${FFMPEG_VERSION_H_TEXT}
    )
...
if(
  ...
  try_compile(
    ...
    ${CMAKE_CURRENT_BINARY_DIR}/ffmpeg_test
    ${CMAKE_CURRENT_LIST_DIR}/ffmpeg_test.cxx
    CMAKE_FLAGS "-DINCLUDE_DIRECTORIES=${FFMPEG_INCLUDE_DIR}"
    LINK_LIBRARIES ${FFMPEG_LIBRARIES}
  )

  if(
    NOT FFMPEG_TEST_COMPILED
...
if(Ffmpeg_FOUND)
  ...
  if(NOT TARGET Ffmpeg::Ffmpeg)
    ...
    set_target_properties(
      Ffmpeg::Ffmpeg
      PROPERTIES INTERFACE_INCLUDE_DIRECTORIES "${FFMPEG_INCLUDE_DIRS}" INTERFACE_LINK_LIBRARIES "${FFMPEG_LIBRARIES}"
    )
  endif()
  if(APPLE)
    find_package(Iconv)
