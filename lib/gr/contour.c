...
static int lookup_table[6][3][2] = {{{0, 1}, {0, 2}}, {{0, 1}, {1, 2}}, {{0, 2}, {1, 2}},
...
static void gradient(int ind, int n, double *xpts, double *ypts, enum contour_op op)
{
  int i, j;
  static int count;
  double t;
  ...
  switch (op)
    ...
    case CT_EVAL:
      ...
      if (contour_vars.gradient_mag[ind] > max_mag) max_mag = contour_vars.gradient_mag[ind];
      break;
    ...
    case CT_END:
      ...
      if (magnitude != NULL) free(magnitude);
      break;
    }
}
...
static void variance(int ind, int n, double *xpts, double *ypts, enum contour_op op)
{
  double y;
  double Sxx, Syy, Sxy;
  ...
  switch (op)
    ...
    case CT_EVAL:
      ...
      if (contour_vars.variance_list[ind] > max_var) max_var = contour_vars.variance_list[ind];
      break;
...
static int find_good_place(int n, double *xpts, double *ypts, double r_sqr)
{
  int i, i_ind;
  int j, j_ind;
  int k;
  ...
  while ((!closed && i < n - 1) || (closed && j_ind != contour_vars.start_index))
    {
      ...
      while (dist < r_sqr)
        {
          ...
          if (++i_ind >= n)
            {
              if (!closed || k == 0) goto avg_done; /* Not enough points for moving average */
              i_ind = 1;
            }
          if (i_ind == k) goto avg_done;
      ...
      if (j >= ind[0])
        {
          if (contour_vars.start_index == -1) contour_vars.start_index = j_ind;
...
static void label_line(int n, double *xpts, double *ypts, double *zpts, char *label)
{
  int i, j, k;
  int error_ind;
  int n_pts;
  ...
  gks_inq_text_extent(contour_vars.wkid, d, e, label, &error_ind, &cpx, &cpy, tx, ty);
  ...
  if (k != -1)
    {
      ...
      x_text_pos = x_text_pos * contour_vars.scale_factor + contour_vars.vp[0];
      y_text_pos = y_text_pos * contour_vars.scale_factor * contour_vars.aspect_ratio + contour_vars.vp[2];
...
static void draw(double x, double y, double z, int iflag)
{
  static int n = 0;
      ...
      if (z_exept_flag == 0)
        {
          ...
          if ((contour_vars.txtflg == 1) &&
              ((contour_vars.lblmjh == 1) || (((iflag / 10 - 1) % contour_vars.lblmjh) == 1)))
            {
              dx = xpts[n] - xpts[n - 1];
              dy = (ypts[n] - ypts[n - 1]) * contour_vars.aspect_ratio;
              line_length += contour_vars.scale_factor * sqrt(dx * dx + dy * dy);
            }
          n++;
          ...
          if ((line_length >= contour_max_length) || (n >= contour_max_pts))
            {
              if ((contour_vars.txtflg == 1) &&
                ...
                {
                  if ((contour_vars.lblmjh <= 1) || (((iflag / 10 - 1) % contour_vars.lblmjh) == 1))
                    {
                      linetype = GKS_K_LINETYPE_SOLID;
                    }
                  else
                  ...
                  if (contour_vars.use_color)
                    {
                      colorind = (int)(1000 + (z - contour_vars.zmin) / (contour_vars.zmax - contour_vars.zmin) * 255);
                      gr_setlinecolorind(colorind);
                    }
      ...
      if (z_exept_flag == 0)
        {
          ...
          if ((line_length >= contour_min_length) && (contour_vars.txtflg == 1) &&
              ((contour_vars.lblmjh == 1) || (((iflag / 10 - 1) % contour_vars.lblmjh) == 1)))
            {
              linetype = GKS_K_LINETYPE_SOLID;
              if (contour_vars.lblmjh > 1)
...
static void calc_contours(double *z, int nrz, int nx, int ny, double *cv, int ncv, double zmax, int *bitmap,
...
L200:
  ...
  xy[3 - l - 1] = ij[3 - l - 1];
  BITMAP(ij[0], ij[1], icv, l) = 1;
  draw(xmin + (xy[0] - 1.0) * dx, ymin + (xy[1] - 1.0) * dy, cval, iflag + 10 * icv);
  if (iflag < 4)
    {
      goto L210;
    }
...
void gr_draw_contours(int nx, int ny, int nh, double *px, double *py, double *h, double *z, int major_h)
{
  double mmin, mmax, *cv;
  int ncv, *bitmap;
  int i, j, k, n = 0;
  ...
  contour_vars.scale_factor = (contour_vars.vp[1] - contour_vars.vp[0]) / (contour_vars.wn[1] - contour_vars.wn[0]);
  contour_vars.aspect_ratio =
      (contour_vars.vp[3] - contour_vars.vp[2]) / (contour_vars.wn[3] - contour_vars.wn[2]) / contour_vars.scale_factor;
  ...
  if ((rotation == 0) && (tilt == 90) && (contour_vars.lblmjh > 0))
    {
      ...
      contour_vars.x_map_size = (int)(contour_map_rate * (contour_vars.vp[1] - contour_vars.vp[0]) / char_height) + 2;
      contour_vars.y_map_size = (int)(contour_map_rate * (contour_vars.vp[3] - contour_vars.vp[2]) / char_height) + 2;
      contour_vars.label_map = (int *)xmalloc(contour_vars.x_map_size * contour_vars.y_map_size * sizeof(int));
      ...
      contour_vars.x_map_factor = (double)(contour_vars.x_map_size - 3) / (contour_vars.wn[1] - contour_vars.wn[0]);
      contour_vars.y_map_factor = (double)(contour_vars.y_map_size - 3) / (contour_vars.wn[3] - contour_vars.wn[2]);
    }
  else
    {
  ...
  if (nh < 1)
    {
      ...
      for (i = 0; i < ncv; i++) cv[i] = mmin + (double)(i) / (ncv - 1) * (mmax - mmin);
    }
  else
    {
      ncv = nh;
  ...
  if (contour_vars.label_map != NULL) free(contour_vars.label_map);
  if (cv != h) free(cv);
}

static int get_lookup_table_index(int *triangle, double *z, double isolevel)
{
  int index, i;
...
static void interpolate_line_segment(double *x, double *y, double *z, int *triangle, double isolevel, int *nlines,
                                     vertex_t *lines)
{
  int indices[2];
  int index, i, j;
      ...
      for (i = 0; i < 2; ++i)
        {
          ...
          lines[i].x = x[indices[0]] +
                       (x[indices[1]] - x[indices[0]]) * ((isolevel - z[indices[0]]) / (z[indices[1]] - z[indices[0]]));
          lines[i].y = y[indices[0]] +
                       (y[indices[1]] - y[indices[0]]) * ((isolevel - z[indices[0]]) / (z[indices[1]] - z[indices[0]]));
        }
      *nlines = 1;
    }
  else
...
static void convert_segments_to_polylines(int nsegments, vertex_t *segments, int *nlines, polyline_t **lines)
{
  polyline_t *lin;
  int nlin;
  polyline_t *cur_lin;
...
static void march_triangles(double *x, double *y, double *z, int ntri, int *triangles, double isolevel, int *nlines,
                            polyline_t **lines)
{
  int i, j;
  vertex_t cur_line[2];
  ...
  for (i = 0; i < ntri; ++i)
    {
      interpolate_line_segment(x, y, z, &triangles[3 * i], isolevel, &cur_nlines, cur_line);
...
void gr_draw_tricont(int npoints, double *x, double *y, double *z, int nlevels, double *levels, int *colors)
{
  int i, l;
  int ntri, *triangles;
  int nlines = 0;
  ...
  for (l = 0; l < nlevels; l++)
    {
      ...
      for (i = 0; i < nlines; i++) gr_polyline(lines[i].npoints, lines[i].x, lines[i].y);
