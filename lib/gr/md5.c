      ...
      if (in_block > MD5_BLOCK_SIZE)
        {
          ...
          memmove(md5_p->md_buffer, md5_p->md_buffer + (in_block & ~BLOCK_SIZE_MASK), in_block & BLOCK_SIZE_MASK);
...
static void md5_finish(md5_t *md5_p, void *signature)
{
  ...
  if (pad <= 0) pad += MD5_BLOCK_SIZE;

  if (pad > 0)
    {
      md5_p->md_buffer[bytes] = (unsigned char)0x80;
      if (pad > 1) memset(md5_p->md_buffer + bytes + 1, 0, pad - 1);
      bytes += pad;
    }
