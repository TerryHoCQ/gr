...
struct ThreadAttr
{
  ...
  int x_diff, y_diff;
  int width, height, new_width;
  std::array<uint32_t, 3> smoothed_colors;
...
static bool mouse_move_triggered = false;
...
static std::list<HighlightMask> mask_highlights;
static std::list<HighlightMask> referred_elements_highlights_cache;
static std::list<HighlightMask> mask_highlights_cache;
...
void getMousePos(QMouseEvent *event, int *x, int *y)
{
  ...
  x[0] = static_cast<int>(event->position().x());
  y[0] = static_cast<int>(event->position().y());
#else
  x[0] = static_cast<int>(event->pos().x());
  y[0] = static_cast<int>(event->pos().y());
#endif
}
...
GRPlotWidget::GRPlotWidget(QMainWindow *parent, int argc, char **argv, bool listen_mode, int listen_port,
                           bool test_mode, const QString &test_commands)
    : QWidget(parent), pixmap(), redraw_pixmap(RedrawType::NONE), args_(nullptr), rubber_band(nullptr)
{
  ...
  keep_window_act->setChecked(false);
  ...
  if (!getenv("GRDISPLAY") || (getenv("GRDISPLAY") && strcmp(getenv("GRDISPLAY"), "view") != 0))
    {
      ...
      hide_log_sub_menu_act = new QAction(this);
      show_log_sub_menu_act = new QAction(this);
      hide_flip_sub_menu_act = new QAction(this);
      show_flip_sub_menu_act = new QAction(this);
      hide_plot_type_sub_menu_act = new QAction(this);
      show_plot_type_sub_menu_act = new QAction(this);
      ...
      show_add_element_act = new QAction();
      ...
      possible_elems_menu = new QMenu(this);
...
static void dilatationForImagePart(void *arg)
{
  ...
  constexpr int box_size = 11, inner_box_size = 5;
  auto attr = static_cast<struct ThreadAttr *>(arg);

  for (unsigned int j = attr->y_start; j < attr->y_end; j++)
    {
      for (unsigned int i = attr->x_start; i < attr->x_end; i++)
        {
          auto found_pixel_distance_from_center = -1;
          for (int box_offset_y = 0; box_offset_y <= box_size / 2; box_offset_y++)
            {
              int pixel_y_0 = j + box_offset_y - attr->y_diff;
              int pixel_y_1 = j - box_offset_y - attr->y_diff;
              if (pixel_y_0 < 0 || pixel_y_0 >= attr->height) pixel_y_0 = -1;
              if (pixel_y_1 < 0 || pixel_y_1 >= attr->height) pixel_y_1 = -1;
              if (pixel_y_0 == -1 && pixel_y_1 == -1) continue;
              for (int box_offset_x = 0; box_offset_x <= box_size / 2; box_offset_x++)
                {
                  int pixel_x_0 = i + box_offset_x - attr->x_diff;
                  int pixel_x_1 = i - box_offset_x - attr->x_diff;
                  if (pixel_x_0 >= 0 && pixel_x_0 < attr->width)
                    {
                      if ((pixel_y_0 != -1 && attr->pixels[pixel_y_0 * attr->width + pixel_x_0] != no_color) ||
                          (pixel_y_1 != -1 && attr->pixels[pixel_y_1 * attr->width + pixel_x_0] != no_color))
                        {
                          found_pixel_distance_from_center = std::min<int>(box_offset_x, box_offset_y);
                          break;
                        }
                    }
                  if (pixel_x_1 >= 0 && pixel_x_1 < attr->width)
                    {
                      if ((pixel_y_0 != -1 && attr->pixels[pixel_y_0 * attr->width + pixel_x_1] != no_color) ||
                          (pixel_y_1 != -1 && attr->pixels[pixel_y_1 * attr->width + pixel_x_1] != no_color))
                        {
                          found_pixel_distance_from_center = std::min<int>(box_offset_x, box_offset_y);
                          break;
                        }
              ...
              if (found_pixel_distance_from_center != -1) break;
            }
          if (found_pixel_distance_from_center >= 0)
            {
              (*attr->image)[j * attr->new_width + i] = color;
              (found_pixel_distance_from_center <= inner_box_size / 2)
                  ? color
                  : attr->smoothed_colors[found_pixel_distance_from_center - inner_box_size / 2 - 1];
            }
        }
    }
}

static int systemProcessorCount(void)
{
#ifdef _WIN32
...
static std::vector<uint32_t> dilatationForImage(unsigned int &x, unsigned int &y, unsigned int &width,
                                                unsigned int &height, const unsigned int *pixels)
{
  constexpr uint32_t no_color = 0xFFFFFFFF;
  constexpr int box_size = 11, inner_box_size = 5;
  constexpr int smoothed_colors_count = std::max(box_size / 2 - inner_box_size / 2, 0);
  std::vector<uint32_t> image;
  std::array<uint32_t, smoothed_colors_count> smoothed_colors;
  auto no_color_rgba = reinterpret_cast<const uint8_t *>(&no_color);
  auto color_rgba = reinterpret_cast<const uint8_t *>(&color);
  ThreadAttr *jobs;
  int x_start = 0, y_start = 0, x_end, y_end;
  int thread_num;
#ifndef NO_THREADS
  ThreadpoolT *thread_pool;
#endif

  for (int i = 0; i < smoothed_colors_count; i++)
    {
      auto smoothed_colors_rgba = reinterpret_cast<uint8_t *>(&smoothed_colors[i]);
      for (int j = 0; j < 4; j++)
        {
          smoothed_colors_rgba[j] =
              (no_color_rgba[j] * (i + 1) + color_rgba[j] * (smoothed_colors_count - i - 1)) / smoothed_colors_count;
        }
    }
  ...
  // Reserve some more space for the dilatation at the image boundary
  auto new_x = std::max<int>(0, x - box_size / 2);
  auto new_y = std::max<int>(0, y - box_size / 2);
  unsigned int new_width = std::min<int>(x + width + box_size / 2, figure_size_x * 2) - new_x;
  unsigned int new_height = std::min<int>(y + height + box_size / 2, figure_size_y * 2) - new_y;

  image.resize(new_width * new_height, 0x00FFFFFF);

  /* creates the threadpool */
#ifndef NO_THREADS
  thread_pool = static_cast<ThreadpoolT *>(calloc(1, sizeof(*thread_pool)));
  if (thread_pool == 0)
    {
      fprintf(stderr, "Can't allocate memory\n");
      return image;
    }
  thread_num = (systemProcessorCount() - 1) < 256 ? systemProcessorCount() - 1 : 256;
  threadpoolCreate(thread_pool, thread_num, dilatationForImagePart);
#endif

  int thread_size = ceil((new_width + new_height) / (2.0 * thread_num));
  int x_size = ceil(new_width / (1.0 * thread_size)), y_size = ceil(new_height / (1.0 * thread_size));
  jobs = static_cast<ThreadAttr *>(malloc(x_size * y_size * sizeof(ThreadAttr)));

  for (int i = 0; i < x_size; i++)
    {
      x_end = static_cast<int>(std::min((i + 1) * thread_size, static_cast<int>(new_width)));
      for (int j = 0; j < y_size; j++)
        {
          /* transfer data for each thread */
          y_end = static_cast<int>(std::min((j + 1) * thread_size, static_cast<int>(new_height)));
          jobs[i + j * x_size].x_start = x_start;
          jobs[i + j * x_size].y_start = y_start;
          jobs[i + j * x_size].x_end = x_end;
          jobs[i + j * x_size].y_end = y_end;
          jobs[i + j * x_size].x_diff = x - new_x;
          jobs[i + j * x_size].y_diff = y - new_y;
          jobs[i + j * x_size].width = width;
          jobs[i + j * x_size].height = height;
          jobs[i + j * x_size].new_width = new_width;
          jobs[i + j * x_size].image = &image;
          jobs[i + j * x_size].pixels = pixels;
          jobs[i + j * x_size].smoothed_colors = smoothed_colors;

#ifndef NO_THREADS
          threadpoolAddWork(thread_pool, jobs + i + j * x_size);
#endif
          y_start = y_end;
        }
      x_start = x_end;
                                  ...
                                  unsigned int *pixels)
{
  ...
  for (const auto &mask : mask_highlights)
    {
      if (mask.id == id)
        {
          contains_id = true;
  ...
  auto mask = HighlightMask(id, x, y, width, height, image);
  if (!contains_id) mask_highlights.emplace_back(mask);
  if (color == 0xFF0202F0)
    mask_highlights_cache.emplace_back(mask);
  else
    referred_elements_highlights_cache.emplace_back(mask);
}
                                            ...
                                            QWidget **line_edit)
{
  ...
  for (const auto &elem : grm_get_document_root()->querySelectorsAll("plot"))
    {
      move_to_plot_vec.push_back(static_cast<std::string>(elem->getAttribute("plot_id")));
    }
  move_to_plot_vec.push_back("new_plot"); // to move a series into a new plot
  move_to_plot_list.reserve(static_cast<int>(move_to_plot_vec.size()));
  for (auto &i : move_to_plot_vec) move_to_plot_list.push_back(i.c_str());

  std::map<std::string, QStringList> attribute_to_list{
      ...
      {"move_to_plot", move_to_plot_list},
  ...
  static_cast<QComboBox *>(*line_edit)->setEditable(true);
  if (attribute_to_list.count(cur_attr_name))
    {
      ...
      for (const auto &elem : list)
        {
          static_cast<QComboBox *>(*line_edit)->addItem(elem.toStdString().c_str());
              ...
              if (!colormap.isNull())
                {
                  ...
                  static_cast<QComboBox *>(*line_edit)->setItemData(cnt++, colormap, Qt::DecorationRole);
                }
            }
          else if (cur_attr_name == "marker_type")
            {
              if (auto marker_type = QPixmap((":/preview_images/marker_types/" + elem.toStdString() + ".png").c_str());
                  !marker_type.isNull())
                {
                  marker_type.setDevicePixelRatio(15);
                  static_cast<QComboBox *>(*line_edit)->setItemData(cnt++, marker_type, Qt::DecorationRole);
                }
            }
          else if (cur_attr_name == "line_type")
            {
              if (auto line_type = QPixmap(
                  ...
                  !line_type.isNull())
                {
                  ...
                  static_cast<QComboBox *>(*line_edit)->setItemData(cnt++, line_type, Qt::DecorationRole);
                }
            }
          else if (cur_attr_name == "fill_style")
            {
              if (auto fill_style = QPixmap(
                  ...
                  !fill_style.isNull())
                {
                  ...
                  static_cast<QComboBox *>(*line_edit)->setItemData(cnt++, fill_style, Qt::DecorationRole);
                }
            }
          else if (cur_attr_name == "font_precision")
            {
              if (auto font_precision = QPixmap((":/preview_images/font_precisions/" + elem.toStdString() +
                  ...
                  !font_precision.isNull())
                {
                  ...
                  static_cast<QComboBox *>(*line_edit)->setItemData(cnt++, font_precision, Qt::DecorationRole);
                }
            }
          else if (cur_attr_name == "font")
            {
              if (auto font =
                  ...
                  !font.isNull())
                {
                  ...
                  static_cast<QComboBox *>(*line_edit)->setItemData(cnt++, font, Qt::DecorationRole);
                }
            }
        }
      auto *completer = new QCompleter(list, this);
      completer->setCaseSensitivity(Qt::CaseInsensitive);
      static_cast<QComboBox *>(*line_edit)->setCompleter(completer);

      if (cur_attr_name == "move_to_plot")
        QObject::connect(static_cast<QComboBox *>(*line_edit), SIGNAL(highlighted(int)), this,
                         SLOT(highlightMoveToPlotSlot(int)));
    }
                                                    ...
                                                    QWidget **line_edit)
{
  ...
  static_cast<QComboBox *>(*line_edit)->addItem(""); // entry to remove the attribute
           ...
           current_selection->getRef()->getAttribute(cur_attr_name).isInt())
    {
      current_text =
          GRM::locationIntToString(static_cast<int>(current_selection->getRef()->getAttribute(cur_attr_name)));
           ...
           current_selection->getRef()->getAttribute(cur_attr_name).isInt())
    {
      if (auto type = static_cast<std::string>(current_selection->getRef()->getAttribute("axis_type")); type == "x")
        current_text =
  ...
  static_cast<QComboBox *>(*line_edit)->setCurrentIndex(index);
  if (cur_attr_name == "colormap") static_cast<QComboBox *>(*line_edit)->setIconSize(QSize(90, 40));
  if (cur_attr_name == "line_type") static_cast<QComboBox *>(*line_edit)->setIconSize(QSize(60, 40));
  if (cur_attr_name == "font_precision") static_cast<QComboBox *>(*line_edit)->setIconSize(QSize(60, 40));
  if (cur_attr_name == "font") static_cast<QComboBox *>(*line_edit)->setIconSize(QSize(60, 40));
}

void GRPlotWidget::attributeSetForComboBox(const std::string &attr_type, const std::shared_ptr<GRM::Element> &element,
      ...
      try
        {
          if (label == "text_align_vertical")
            ...
            {
              if (auto type = static_cast<std::string>(current_selection->getRef()->getAttribute("axis_type"));
                  type == "x")
...
void GRPlotWidget::redraw(bool full_redraw, bool update_tree)
{
  ...
  if (ignore_resize == 0) // resize triggerd by dockwidgets shouldn't clear the cache since the window stays the same
    {
...
void GRPlotWidget::paint(QPaintDevice *paint_device)
{
          ...
          if (redraw_pixmap == RedrawType::FULL)
            {
              ...
              gr_clearbackground();
                   ...
                   active_plot_through_update != nullptr ||
                   active_figure->querySelectors("plot[_active=\"1\"]") != nullptr)
            {
              double viewport[4];
              int width, height;
              ...
              gr_setbackground(viewport[0], viewport[1], std::max(0.0, height - viewport[3]),
                               std::max(0.0, height - viewport[3]) + viewport[3] - viewport[2]);
            }
        }
  ...
  auto old_selected_elem = grm_get_document_root()->querySelectors("[_highlighted=\"1\"]");
  if (old_selected_elem == nullptr) old_selected_elem = prev_highlighted_tick_group_elem;
  if (old_selected_elem != nullptr)
    {
      auto bbox_id = static_cast<int>(old_selected_elem->getAttribute("_bbox_id"));
      auto bbox_x_min = static_cast<double>(old_selected_elem->getAttribute("_bbox_x_min"));
      auto bbox_x_max = static_cast<double>(old_selected_elem->getAttribute("_bbox_x_max"));
      auto bbox_y_min = static_cast<double>(old_selected_elem->getAttribute("_bbox_y_min"));
      auto bbox_y_max = static_cast<double>(old_selected_elem->getAttribute("_bbox_y_max"));
      auto *bbox = new BoundingObject(bbox_id, bbox_x_min, bbox_x_max, bbox_y_min, bbox_y_max, old_selected_elem);
          ...
          if (tooltip.xPx() > 0 && tooltip.yPx() > 0)
            {
              ...
              if (auto plot_element = grm_get_subplot_from_ndc_points_using_dom(1, &x, &y);
                  plot_element != nullptr && plot_element->hasAttribute("_kind"))
  ...
  if (enable_editor)
    {
      ...
      if (mouse_state.mode == MouseState::Mode::MOVE_SELECTED)
        ...
        {
          auto elements_at_pos = bounding_logic->getElementsAtPoint(x, y, hide_grid_bbox, enable_advanced_editor);

          if (current_selection == nullptr)
            {
              auto global_root = grm_get_document_root();
              mouse_move_selections.clear();
              if (!elements_at_pos.empty())
                {
                  for (const auto &set_elem : elements_at_pos)
                    {
                      mouse_move_selections.emplace(set_elem);
                    }
                }
              update();
            }
  ...
  else
    {
      ...
      if (mouse_state.mode == MouseState::Mode::BOXZOOM)
      ...
      else
        {
          ...
          if (plot_element)
            {
              if (auto kind = static_cast<std::string>(plot_element->getAttribute("_kind")); kind == "marginal_heatmap")
                {
          ...
          else
            {
              for (const auto &plot_elem : global_root->querySelectorsAll("plot"))
                {
                  if (auto kind = static_cast<std::string>(plot_elem->getAttribute("_kind"));
...
void GRPlotWidget::possibleElementsMenuSlot()
{
  auto act = qobject_cast<QAction *>(sender());
  int bbox_id = act->data().value<int>();

  auto global_root = grm_get_document_root();
  auto elem = global_root->querySelectors("[_bbox_id=\"" + std::to_string(bbox_id) + "\"]");
  ...
  auto *bbox = new BoundingObject(bbox_id, bbox_x_min, bbox_x_max, bbox_y_min, bbox_y_max, elem);
  ...
  for (const auto &e : grm_get_document_root()->querySelectorsAll("[_highlighted=\"1\"]"))
    {
      e->removeAttribute("_highlighted");
    }
...
void GRPlotWidget::highlightMoveToPlotSlot(int index)
{
  std::vector<std::string> move_to_plot_vec;
  for (const auto &elem : grm_get_document_root()->querySelectorsAll("plot"))
    {
  ...
  move_to_plot_vec.push_back("new_plot"); // to move a series into a new plot
  move_to_plot_vec.push_back("");

  auto new_plot_id = move_to_plot_vec[index];
  if (new_plot_id != "new_plot" && new_plot_id != "")
    plot_id = new_plot_id;
  else
    plot_id = "";
  redraw();
...
void GRPlotWidget::mousePressEvent(QMouseEvent *event)
{
  ...
  if (event->button() == Qt::MouseButton::RightButton)
  ...
  else if (event->button() == Qt::MouseButton::LeftButton)
    {
      ...
      else if (current_selection != nullptr)
        {
          if (auto elem_name = current_selection->getRef()->localName();
      ...
      if (enable_editor)
        {
              ...
              for (const auto &set_elem : elements_at_pos)
                {
                  ...
                  auto bbox_y_max = static_cast<double>(elem->getAttribute("_bbox_y_max"));
                  auto *bbox =
                      new BoundingObject((int)(set_elem), bbox_x_min, bbox_x_max, bbox_y_min, bbox_y_max, elem);
                  cur_clicked.emplace_back(*bbox);

                  if (old_current_selection == nullptr)
                    {
                      if (elements_at_pos.size() > 1)
                        {
                          auto act = new QAction(elem->localName().c_str());
                          act->setData((int)(set_elem));
                          connect(act, &QAction::triggered, this, &GRPlotWidget::possibleElementsMenuSlot);
                          possible_elems_menu->addAction(act);
                        }
                      else if (elements_at_pos.size() == 1)
                        {
                          current_selection = bbox;
                        }
                    }
          ...
          if (current_selection == nullptr)
            {
              possible_elems_menu->clear();
              possible_elems_menu->close();
              return;
            }
          current_selection->getRef()->setAttribute("_highlighted", true);
          if (auto name = current_selection->getRef()->localName();
              name == "tick_group" || name == "tick" || name == "grid_line" || name == "text")
            {
          ...
          if (ctrl_key_mode)
            {
              ...
              if (!removed_selection)
                {
                  if (auto elem_name = tmp->getRef()->localName();
                      (enable_advanced_editor ||
                       (elem_name == "figure" || elem_name == "plot" || elem_name == "layout_grid" ||
                        elem_name == "layout_grid_element" || elem_name == "colorbar" || elem_name == "label" ||
                        elem_name == "titles_3d" || elem_name == "text" || elem_name == "central_region" ||
              ...
              if (viewport[0] <= ndc_x && ndc_x <= viewport[1] && viewport[2] <= ndc_y && ndc_y <= viewport[3])
                {
                  ...
                  plot_elem->setAttribute("_selected_for_menu", true);
                  plot_elem->setAttribute("_active", true);
                  adjustPlotTypeMenu(plot_elem);
                }
            }
        }
    }
...
{
  const auto global_root = grm_get_document_root();
  if (auto figure = global_root->querySelectors("figure[active=1]"); figure != nullptr)
    {
...
{
  if (event->angleDelta().y() != 0 && !enable_editor)
    {
      int x, y;
      ...
      grm_args_push(args, "x", "i", x);
      grm_args_push(args, "y", "i", y);
      grm_args_push(args, "angle_delta", "d", static_cast<double>(event->angleDelta().y()));
      grm_input(args);
      grm_args_delete(args);
...
void GRPlotWidget::volume()
{
  ...
  if (current_selection != nullptr && current_selection->getRef() != nullptr)
    current_selection->getRef()->removeAttribute("highlighted");
  current_selection = nullptr;
  current_selections.clear();
  edit_element_widget->hide();
  hide_add_element_act->trigger();
...
void GRPlotWidget::xLimSlot()
{
  ...
  QDialog dialog(this);
  dialog.setWindowTitle("Set X Lim:");
  auto form = new QFormLayout;
...
void GRPlotWidget::yLimSlot()
{
  ...
  QList<QLineEdit *> fields;
  QDialog dialog(this);
  dialog.setWindowTitle("Set Y Lim:");
  auto form = new QFormLayout;
  ...
  if (dialog.exec() == QDialog::Accepted)
    {
      ...
      if (fields[0]->isModified())
        {
          if (!fields[0]->text().isEmpty())
            plot_elem->setAttribute("y_lim_min", std::stod(fields[0]->text().toStdString()));
          else if (plot_elem->hasAttribute("y_lim_min"))
            plot_elem->removeAttribute("y_lim_min");
        }
      if (fields[1]->isModified())
        {
          if (!fields[1]->text().isEmpty())
            plot_elem->setAttribute("y_lim_max", std::stod(fields[1]->text().toStdString()));
          else if (plot_elem->hasAttribute("y_lim_max"))
            plot_elem->removeAttribute("y_lim_max");
        }
    }
...
void GRPlotWidget::zLimSlot()
{
  ...
  QList<QLineEdit *> fields;
  QDialog dialog(this);
  dialog.setWindowTitle("Set Z Lim:");
  auto form = new QFormLayout;
...
void GRPlotWidget::twinYAxisSlot()
{
  ...
  if (const auto coordinate_system = figure_elem->querySelectors("coordinate_system"))
    {
      const auto axis = coordinate_system->querySelectors("axis[location=\"twin_y\"]");
  ...
  if (enable_editor)
    {
      ...
      for (const auto &cur_child : global_root->querySelectorsAll("[_bbox_id]"))
        {
          auto id = static_cast<int>(cur_child->getAttribute("_bbox_id"));
          auto xmin = static_cast<double>(cur_child->getAttribute("_bbox_x_min"));
          auto xmax = static_cast<double>(cur_child->getAttribute("_bbox_x_max"));
          auto ymin = static_cast<double>(cur_child->getAttribute("_bbox_y_min"));
          auto ymax = static_cast<double>(cur_child->getAttribute("_bbox_y_max"));
  ...
  if (enable_editor)
    {
      ...
      if (!ctrl_key_mode)
        {
          ...
          else if (!mouse_move_selections.empty())
            {
              const auto global_root = grm_get_document_root();
              const auto render = grm_get_render();
              for (const auto &id : mouse_move_selections)
                {
                  auto elem = global_root->querySelectors("[_bbox_id=\"" + std::to_string(id) + "\"]");
                  if (elem == nullptr) continue;
                  if ((elem->localName() == "side_region" || elem->localName() == "central_region" ||
                       elem->localName() == "figure" || elem->localName() == "plot" ||
                       elem->localName() == "layout_grid_element" || elem->localName() == "layout_grid" ||
                       elem->localName() == "overlay") &&
                      !elem->hasChildNodes())
                    continue;
                  bool contains_id = false;
                  for (const auto &mask : mask_highlights_cache)
                    {
                      if (mask.id == id)
                        {
                          contains_id = true;
                          mask_highlights.emplace_back(mask);
                          break;
                        }
                    }
                  if (!contains_id) render->renderMaskHighlight(elem, processPartialDrawing);
                }

              if (!mask_highlights.empty())
                {
                  for (const auto &mask : mask_highlights)
                    {
                      auto elem = global_root->querySelectors("[_bbox_id=\"" + std::to_string(mask.id) + "\"]");

                      // don't highlight figure and plot if they have the same mask (equals just 1 plot in the window)
                      if (elem->localName() == "figure" && global_root->querySelectors("layout_grid") == nullptr)
                        continue;
                      if (elem->localName() == "layout_grid") continue;
                      // maybe add special case for layout_grid_element and plot, which will require a more tricky check
                      painter.setOpacity(0.2);
                      auto rect =
                          QRect(mask.x / this->devicePixelRatioF(), mask.y / this->devicePixelRatioF(),
                                mask.width / this->devicePixelRatioF(), mask.height / this->devicePixelRatioF());
                      auto image = QImage(mask.image.data(), mask.width, mask.height, QImage::Format_ARGB32);
                      painter.drawImage(rect, image);
                      painter.setOpacity(1.0);

                      if (elem != nullptr && enable_advanced_editor)
                        {
                          int x_pos = 5, y_pos = 10;
      ...
      if (!tree_selections.empty())
        {
          const auto global_root = grm_get_document_root();
          const auto render = grm_get_render();
          for (const auto &id : tree_selections)
            {
              auto elem = global_root->querySelectors("[_bbox_id=\"" + std::to_string(id) + "\"]");
              if (elem == nullptr) continue;
              if ((elem->localName() == "side_region" || elem->localName() == "central_region" ||
              ...
              for (const auto &mask : mask_highlights)
                {
                  ...
                  // don't highlight figure and plot if they have the same mask (equals just 1 plot in the window)
                  if (elem->localName() == "figure" && global_root->querySelectors("layout_grid") == nullptr) continue;
                  if (elem->localName() == "layout_grid") continue;
                  // maybe add special case for layout_grid_element and plot, which will require a more tricky check
                  painter.setOpacity(0.2);
                  auto rect = QRect(mask.x / this->devicePixelRatioF(), mask.y / this->devicePixelRatioF(),
                                    mask.width / this->devicePixelRatioF(), mask.height / this->devicePixelRatioF());
                  auto image = QImage(mask.image.data(), mask.width, mask.height, QImage::Format_ARGB32);
                  painter.drawImage(rect, image);
                  painter.setOpacity(1.0);
      ...
      if (selected_parent != nullptr)
        {
          ...
          if (selected_parent->getRef() != nullptr)
            {
              auto id = selected_parent->getId();
              bool contains_id = false;
              color = 0xFFFF0000;
              auto name = selected_parent->getRef()->localName();
              if ((name != "side_region" && name != "central_region" && name != "figure" || name != "plot" ||
                   name != "layout_grid_element" || name != "layout_grid" || name != "overlay") ||
                  selected_parent->getRef()->hasChildNodes())
                {
                  for (const auto &mask : mask_highlights_cache)
                    {
                      if (mask.id == id)
                  ...
                  if (!contains_id) render->renderMaskHighlight(selected_parent->getRef(), processPartialDrawing);
      ...
      if (!referenced_elements.empty())
        {
          ...
          auto render = grm_get_render();
          color = 0xFFF3E03B;
          for (const auto &ref_elem : referenced_elements)
            {
              auto id = static_cast<int>(ref_elem.getRef()->getAttribute("_bbox_id"));
              auto elem = global_root->querySelectors("[_bbox_id=\"" + std::to_string(id) + "\"]");
              if (elem == nullptr || (elem->hasAttribute("_selected_for_move") &&
                                      static_cast<int>(elem->getAttribute("_selected_for_move")) == 0))
                continue;
              if ((elem->localName() == "side_region" || elem->localName() == "central_region" ||
                   elem->localName() == "figure" || elem->localName() == "plot" ||
                   elem->localName() == "layout_grid_element" || elem->localName() == "layout_grid" ||
                ...
                continue;
              bool contains_id = false;
              for (const auto &mask : referred_elements_highlights_cache)
                {
                  if (mask.id == id)
                    {
                      contains_id = true;
      ...
      // special case for hovering over elements of move_to_plot combobox to see which plot has that id
      if (plot_id != "")
        {
          auto global_root = grm_get_document_root();
          auto render = grm_get_render();
          auto plot = global_root->querySelectors("plot[plot_id=\"" + plot_id + "\"]");
          auto id = static_cast<int>(plot->getAttribute("_bbox_id"));
          ...
          if (!contains_id) render->renderMaskHighlight(plot, processPartialDrawing);
      ...
      if (auto global_root = grm_get_document_root(); global_root->querySelectors("[_selected_for_move=\"1\"]"))
        {
          auto render = grm_get_render();
          for (const auto selection : global_root->querySelectorsAll("[_selected_for_move=\"1\"]"))
            {
              auto id = static_cast<int>(selection->getAttribute("_bbox_id"));
              auto elem = global_root->querySelectors("[_bbox_id=\"" + std::to_string(id) + "\"]");
              if (elem == nullptr) continue;
              if ((elem->localName() == "side_region" || elem->localName() == "central_region" ||
                   elem->localName() == "figure" || elem->localName() == "plot" ||
                ...
                continue;
              bool contains_id = false;
              // tooltip at mouse-pos with name? or name somewhere else? figure and plot are highlighted together,
              // how is that visible understandable
              for (const auto &mask : mask_highlights_cache)
                {
                  if (mask.id == id)
                    {
    ...
    {
      const auto global_root = grm_get_document_root();
      if (auto plot_elem = global_root->querySelectors("[_selected_for_menu=1]"); plot_elem != nullptr)
        {
          int width, height;
          double mwidth, mheight;
  ...
  if (editor_action->text() == "&Enable Editorview")
    {
      ...
      enable_editor = true;
      render->setEnableEditor(true);
      grm_tmp_dir = render->initializeHistory();
      ...
      // to fix a strange behaviour on macos
      for (const auto &plot_elems : root->querySelectorsAll("plot[_active=\"1\"]"))
        {
          plot_elems->removeAttribute("_active");
        }
      for (const auto &plot_elems : root->querySelectorsAll("plot[_active_through_update=\"1\"]"))
        {
          plot_elems->removeAttribute("_active_through_update");
        }
    ...
    {
      enable_editor = false;
      grm_get_render()->setEnableEditor(false);
      enable_advanced_editor = false;
      advanced_editor_act->setVisible(false);
      add_element_action->setVisible(false);
      show_container_action->setVisible(false);
...
void GRPlotWidget::addContextSlot()
{
  ...
  // convert the data
  auto context = grm_get_render()->getContext();
  if (!grm_context_data_from_file(context, path))
    {
...
void GRPlotWidget::addGRPlotDataContextSlot()
{
  ...
  // convert the data
  auto context = grm_get_render()->getContext();
  if (!grm_context_data_from_file(context, path, true))
    {
      fprintf(stderr, "Could not interpret the file to context data\n");
      return;
    }
...
void GRPlotWidget::generateLinearContextSlot()
{
  ...
  QDialog dialog(this);
  dialog.setWindowTitle("Generate linear context entry");
  auto form = new QFormLayout;
  ...
  if (dialog.exec() == QDialog::Accepted)
    {
      ...
      try
        {
          auto start = std::stod(values[1]);
          auto end = std::stod(values[2]);
          auto n = std::stoi(values[3]);
...
{
  // TODO: Get Plot ID
  if (auto event_size = QSize(new_size_object->size_event.width, new_size_object->size_event.height);
      this->size() != event_size)
    {
  ...
  while (test_commands_stream && !test_commands_stream->atEnd())
    {
      ...
      if (QStringList words = line.split(","); !words.empty())
        {
          ...
          if (words[0] == "setArg" && words.size() >= 4)
            ...
            {
#ifndef NO_XERCES_C
              if (auto file = fopen(words[1].toStdString().c_str(), "r"))
                {
                  grm_load_graphics_tree(file);
                  tooltips.clear();
                  redraw();
            ...
            {
              if (std::ofstream save_file_stream(words[1].toStdString()); save_file_stream)
                {
...
void GRPlotWidget::setCurrentSelection(BoundingObject *p_current_selection)
{
  ...
  plot_parent->setAttribute("_active", true);
  render->setAutoUpdate(auto_update);
}
...
const QStringList &GRPlotWidget::getCheckBoxAttributes()
{
  return check_box_attr;
}

const QStringList &GRPlotWidget::getComboBoxAttributes()
{
  ...
  if (move_to_plot)
    {
      move_to_plot = false;
      for (const auto &plot :
           grm_get_render()->getActiveFigure()->querySelectorsAll("plot[_active_through_update=\"1\"]"))
        {
          plot->removeAttribute("_active_through_update");
        }
    }
...
void GRPlotWidget::setReferencedElements(const std::vector<BoundingObject> &referenced_elements)
{
  ...
  if (tree_widget != nullptr) // dummy elem which only exist in default grplot case
    {
      ...
      if (colormap_act->isVisible() && plot_parent->hasAttribute("colormap"))
        {
          std::string colormap_string;
          if (plot_parent->getAttribute("colormap").isInt())
            colormap_string = GRM::colormapIntToString(static_cast<int>(plot_parent->getAttribute("colormap")));
          else
            colormap_string = static_cast<std::string>(plot_parent->getAttribute("colormap"));
          auto colormap = QPixmap((":/preview_images/colormaps/" + colormap_string + ".png").c_str());
          colormap_act->setIcon(colormap.scaled(20, 20));
        }
          ...
          for (const auto &series : series_elements)
            {
              ...
              if (auto kind = static_cast<std::string>(series->getAttribute("kind"));
                  kind == "contour" || kind == "heatmap" || kind == "imshow" || kind == "marginal_heatmap" ||
              ...
              else if (kind == "polar_heatmap" || kind == "polar_histogram")
                {
                  ...
                  y_log_act->setVisible(false);
                  hide_lim_sub_menu_act->trigger();
                  hide_plot_type_sub_menu_act->trigger();
                }
              ...
              else if (kind == "pie")
                {
                  x_log_act->setVisible(false);
                  y_log_act->setVisible(false);
                  x_flip_act->setVisible(false);
                  y_flip_act->setVisible(false);
                  hide_log_sub_menu_act->trigger();
                  hide_flip_sub_menu_act->trigger();
                  hide_lim_sub_menu_act->trigger();
                  hide_plot_type_sub_menu_act->trigger();
                }
              else if (kind == "quiver")
                {
                  only_square_aspect_ratio_act->setVisible(false);
                  hide_plot_type_sub_menu_act->trigger();
                }
...
QAction *GRPlotWidget::getHideLogSubMenuAct()
{
  return hide_log_sub_menu_act;
}

QAction *GRPlotWidget::getShowLogSubMenuAct()
{
  return show_log_sub_menu_act;
}

QAction *GRPlotWidget::getHideFlipSubMenuAct()
{
  return hide_flip_sub_menu_act;
}

QAction *GRPlotWidget::getShowFlipSubMenuAct()
{
  return show_flip_sub_menu_act;
}

QAction *GRPlotWidget::getHidePlotTypeSubMenuAct()
{
  return hide_plot_type_sub_menu_act;
}

QAction *GRPlotWidget::getShowPlotTypeSubMenuAct()
{
...
QAction *GRPlotWidget::getShowAddElementAct()
{
  return show_add_element_act;
}
...
QAction *GRPlotWidget::getHideAddElementAct()
{
  return hide_add_element_act;
}
...
QWidget *GRPlotWidget::getAddElementWidget()
{
  return add_element_widget;
}
...
{
  const auto context = grm_get_render()->getContext();
  table_widget->updateData(context);
  table_widget->show();
  show_table_widget_act->trigger();
  ignore_resize = 2;
...
void GRPlotWidget::overlayElementEdit()
{
  ...
  if (auto text = overlay_element->querySelectors("text"); text != nullptr)
    {
...
void GRPlotWidget::listItemCheckStatusChanged(QListWidgetItem *item)
{
  if (item->checkState() == Qt::Unchecked)
    {
      static_cast<CustomQListWidgetItem *>(item)->getElementRef()->removeAttribute("_selected_for_move");
...
{
  auto selection = static_cast<CustomQListWidgetItem *>(item)->getElementRef();
...
void GRPlotWidget::addTreeSelection(int id)
{
  tree_selections.clear();
  this->tree_selections.emplace(id);
}

void GRPlotWidget::setMoveToPlot(bool status)
{
  move_to_plot = status;
}

void GRPlotWidget::changeIconBarVisibility(bool visibility)
{
  if (visibility)
    {
      show_plot_type_sub_menu_act->trigger();
      ...
      show_orientation_sub_menu_act->trigger();
      show_aspect_ratio_sub_menu_act->trigger();
      show_location_sub_menu_act->trigger();
    }
  else
    {
      hide_plot_type_sub_menu_act->trigger();
